...
async function generate() {
  ...
  // Format as TS array
  let tsContent = `import { Post } from '../types';\n\nexport const hrRecruitmentArticles_${index}: Post[] = [\n`;
  ...
  id: '${a.topic.id}',
  slug: '${a.topic.slug}',
  title: \`${a.h1.replace(/`/g, "'")}\`,
  excerpt: \`${a.meta ? a.meta.replace(/`/g, "'") : a.h1.replace(/`/g, "'")}\`,
  metaDescription: \`${a.meta ? a.meta.replace(/`/g, "'") : a.h1.replace(/`/g, "'")}\`,
  category: 'HR',
  ...
  fs.writeFileSync(`src/data/hrRecruitmentArticles_${index}.ts`, tsContent);
  console.log(`Wrote src/data/hrRecruitmentArticles_${index}.ts`);
}
