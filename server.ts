...
async function startServer() {
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      // We'll format the messages for the chat model
      // The Gemini API format expects an array of content objects matching { role: 'user' | 'model', parts: [{ text: string }] }
      const history = messages.map(msg => ({
        role: msg.role === 'user' ? 'user' : 'model',
        parts: [{ text: msg.content }]
      }));

      // Pull off the last user message to pass to sendMessage
